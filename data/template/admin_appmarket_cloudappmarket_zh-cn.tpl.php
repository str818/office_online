<?php if (!defined('IN_DZZ')) exit('Access Denied'); /*a:9:{s:74:"/opt/lampp/htdocs/dzzoffice//./admin/appmarket/template/cloudappmarket.htm";i:1585620672;s:83:"/opt/lampp/htdocs/dzzoffice//./core/template/default/common/header_simple_start.htm";i:1585620676;s:81:"/opt/lampp/htdocs/dzzoffice//./core/template/default/common/header_simple_end.htm";i:1585620676;s:77:"/opt/lampp/htdocs/dzzoffice//./core/template/default/common/commer_header.htm";i:1585620676;s:61:"/opt/lampp/htdocs/dzzoffice//./admin/template/header_left.htm";i:1585620673;s:73:"/opt/lampp/htdocs/dzzoffice//./admin/appmarket/template/header_search.htm";i:1585620672;s:76:"/opt/lampp/htdocs/dzzoffice//./core/template/default/common/header_right.htm";i:1585620676;s:64:"/opt/lampp/htdocs/dzzoffice//./admin/appmarket/template/left.htm";i:1585620672;s:77:"/opt/lampp/htdocs/dzzoffice//./core/template/default/common/footer_simple.htm";i:1585620676;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    ...
    <script type="text/javascript" src="./data/template/admin_appmarket_cloudappmarket_header_zh-cn.js"></script>
    ...
    <script type="text/javascript" src="./data/template/admin_appmarket_cloudappmarket_common_zh-cn.js"></script>
...
<nav class="navbar navbar-inverse resNav bs-top-container">
    <div class="resNav-item resNav-left">
        <ul class="nav navbar-nav navbar-nav-left" style="min-width:168px">
            <li>
                <a class="leftTopmenu" href="index.php?mod=appmanagement" style="padding:8px">
                    <div class="gb_fc"><span class="dzz dzz-chevron-left" style="display:block"></span></div>
                </a>
            </li>
            <li>
    ...
    <div class="resNav-item resNav-center"><?php if ($_GET['op'] == 'extopen') { ?>
            <div class="input-search">
                <form name="search" action="<?php echo BASESCRIPT; ?>" method="get">
                    ...
                    <input type="text" class="form-control search  form-search" name="ext"
                           value="<?php echo $_GET['ext']; ?>" placeholder="扩展名" id="searchval">
        ...
        <?php } elseif (empty($_GET['op'])) { ?>
            <div class="input-search">
                <form name="search" action="<?php echo BASESCRIPT; ?>" method="get">
                    ...
                    <i class="input-search-icon glyphicon glyphicon-search" aria-hidden="true"
                       onclick="this.parentNode.submit()"></i>
                    <input type="text" class="form-control search  form-search" name="keyword"
                           value="<?php echo $_GET['keyword']; ?>" placeholder="应用名称或供应商" id="searchval">
                    <span aria-hidden="true" id="emptysearchcondition" class="header-closebutton">×</span>
            ...
            <div class="input-search">
                <form name="search" action="<?php echo BASESCRIPT; ?>" method="get">
                    ...
                    <i class="input-search-icon glyphicon glyphicon-search" aria-hidden="true"
                       onclick="this.parentNode.submit()"></i>
                    <input type="text" class="form-control search  form-search" name="keyword"
                           value="<?php echo $_GET['keyword']; ?>" placeholder="应用市场" id="searchval">
                    <span aria-hidden="true" id="emptysearchcondition" class="header-closebutton">×</span>
                </form>
            </div>
...
<div class="bs-container clearfix">
    <div class="bs-left-container  clearfix">
        <ul class="nav-stacked">
            ...
            <li <?php if ($op == 'upgrade_app' || $op == 'appupgrade') { ?>class="active"<?php } ?>>
                <a href="<?php echo MOD_URL; ?>&op=appupgrade&refer=<?php echo $refer; ?>"><i
                            class="glyphicon glyphicon-upload"></i> 升级 <span id="update_app_num"
                                                                             class="badge badge-danger <?php if (($upsum = get_update_app_num()) < 1) { ?>hide<?php } ?>"><?php echo $upsum; ?></span></a>
            </li>
    ...
    <div class="bs-main-container">
        <div class="all-center-top clearfix">
            <div class="app-info pull-left"><a href="<?php echo BASESCRIPT; ?>?mod=appmarket">共<span
                            class="num"><?php echo $total; ?></span>个应用</a>

            </div>
            <div class="pull-right" style="color:#999;line-height: 34px;">注：如不能在线安装，请通过 <a class="num"
                                                                                           href="http://www.dzzoffice.com/index.php?mod=dzzmarket"
                                                                                           target="_blank">官方应用市场</a>
                下载应用安装包手动下载
            </div>
        </div>
        <div class="main-content clearfix">
            <div class="market-container" id="loopcontent" style="min-width:510px;">
                <?php if (is_array($list)) foreach ($list as $value) { ?>
                    <div id="app_div_<?php echo $value['mid']; ?>" class="row app_upgradelist">
                        <div id="progess_<?php echo $value['mid']; ?>" class="progess"><span
                                    id="upgrade_progess_<?php echo $value['mid']; ?>" class="upgrade_progess"></span>
                        </div>
                        <div class="col-md-5  col-sm-5 col-xs-5" style="padding-left: 4px;">
                            <div class="app-name-wrapper"><a href="javascript:;"
                                                             data-href="<?php echo $cloudurl; ?>?mod=dzzmarket&op=ajax&do=view&mid=<?php echo $value['mid']; ?>&refer=<?php echo $refer; ?>"
                                                             class="appicon view-detail"
                                                             title="<?php echo $value['name']; ?>"><img
                                            src="<?php echo $value['coverimg']; ?>" style="margin:0"/></a>
                                <p class="appname"><a href="javascript:;"
                                                      data-href="<?php echo $cloudurl; ?>?mod=dzzmarket&op=ajax&do=view&mid=<?php echo $value['mid']; ?>&refer=<?php echo $refer; ?>"
                                                      target="_blank" class="view-detail"
                                                      title="<?php echo $value['name']; ?>"><?php echo $value['name']; ?></a>
                                    <small class="text-muted" title="版本"><?php echo $value['version']; ?></small></p>
                                <div class="appdesc"
                                     title="<?php echo $value['appdesc']; ?>"><?php echo $value[desc_short] ? $value[desc_short] : lang('none'); ?></div>
                            </div>
                        </div>
                        ...
                        <div class="col-md-3  col-sm-3 col-xs-3">
                            <?php if ($value['price'] > 0) { ?>
                                <button class="btn btn-danger"
                                        onclick="showBuyInfo(this,'<?php echo $value['mid']; ?>')"
                                        data-href="<?php echo $cloudurl; ?>?mod=dzzmarket&op=ajax&do=getBuyInfo&mid=<?php echo $value['mid']; ?>"
                                        title="联系购买">购买
                                </button>
                            <?php } else { ?>
                            <?php if ($value['package']) { ?>
                            <?php if ($value["local_appinfo"]) { ?>
                            <btn class="btn btn-default" disabled style="color: #333;"
                                 data-mid="<?php echo $value['mid']; ?>" title="已安装">
                                已安装
                                </button>
                                <?php } else { ?>
                                    <button class="btn btn-success-outline" style="white-space: inherit;"
                                            onclick="start_check_install('<?php echo MOD_URL; ?>&op=install_app_ajax&operation=check_install&appid=<?php echo $value['identifier']; ?>','<?php echo $value['mid']; ?>','<?php echo $value['baseinfo']; ?>');"
                                            id="upgrade_info_<?php echo $value['mid']; ?>">一键安装
                                    </button>
                                <?php } ?>
                                <?php } else { ?>
                                    <a class="btn btn-success-outline view-detail" title="<?php echo $value['name']; ?>"
                                       href="javascript:;"
                                       data-href="<?php echo $cloudurl; ?>?mod=dzzmarket&op=ajax&do=view&mid=<?php echo $value['mid']; ?>">查看详细</a>
                                <?php } ?>
                                <?php } ?>
                        </div>
                    </div>
...
<?php } if (!isset($_G['cookie']['upgradenotice'])) { ?>
    <script type="text/javascript">
        jQuery(document).ready(function () {
            try {
                jQuery('#systemNotice').load('misc.php?mod=upgrade&action=upgradenotice');
            } catch (e) {
            }
            ;
        });
    </script>
    <div id="systemNotice" class="systemNotice"
         style="position: fixed;right:10px;bottom:10px;max-width:50%;box-shadow:0px 5px 10px RGBA(0,0,0,0.3);z-index:999999"></div>
<?php }
}
