...
<head>
    ...
    <script type="text/javascript" src="./data/template/user_login_login_single1_header_zh-cn.js"></script>
    ...
    <![endif]-->
    <script type="text/javascript" src="./data/template/user_login_login_single1_login_zh-cn.js"></script>
    <script type="text/javascript" src="user/scripts/login.js?<?php echo VERHASH; ?>"></script>
    <script type="text/javascript" src="core/util/watermask.js?"></script>
    <script type="text/javascript" src="./data/template/user_login_login_single1_common_zh-cn.js"></script>
...
<div id="wrapper_div"
     style="width: 100%;height:100%;  position: absolute; top: 0px; left: 0px; margin: 0px; padding: 0px; overflow: hidden;z-index:0;  font-size: 0px; background:<?php echo $_G['setting']['loginset']['bcolor'] ? $_G['setting']['loginset']['bcolor'] : '#566481'; ?>;">
    <?php if ($_G['setting']['loginset']['img']) { ?>
        <img src="<?php echo $_G['setting']['loginset']['img']; ?>" name="imgbg" id="imgbg"
             style="right: 0px; bottom: 0px; top: 0px; left: 0px; z-index:10;margin:0;padding:0;overflow:hidden; position: absolute;width:100%;height:100%"
             height="100%" width="100%">
    <?php } ?>
    <?php if ($_G['setting']['loginset']['url']) { ?>
        <iframe id="wrapper_frame" name="wrapper_frame" src="<?php echo $_G['setting']['loginset']['url']; ?>"
                frameborder="0" marginheight="0" marginwidth="0" width="100%" height="100%" allowtransparency="true"
                style="z-index:20;position:absolute;width:100%;height:100%;background:url(dzz/images/b.gif);"></iframe>
    <?php } ?>
</div>
                    ...
                    <form method="post" name="login" id="loginform_<?php echo $loginhash; ?>" class="loginForm"
                        ...
                        <?php } else { ?>
                            <div class="form-group">
                                <input type="text" class="form-control" id="email_<?php echo $loginhash; ?>"
                                       placeholder="邮箱或用户名" name="email" autocomplete="off">
                            </div>
                            <div class="form-group">
                                <input type="text" class="form-control" id="password3_<?php echo $loginhash; ?>"
                                       placeholder="登录密码" name="password" onfocus="this.type='password'"
                                       autocomplete="off">
                            </div>
                        <?php } ?>
...
EOF;
                        ...
                        <div class="form-group  clearfix">
                            <div class="checkbox-custom checkbox-primary pull-left">
                                <input type="checkbox" name="cookietime" id="cookietime_<?php echo $loginhash; ?>"
                                       value="2592000" <?php echo $cookietimecheck; ?> />
                                <label for="cookietime_<?php echo $loginhash; ?>">30天内自动登录</label>
                            </div>
                            <?php if ($_G['setting']['regstatus'] > 0) { ?>
                                <a class="pull-right"
                                   href="user.php?mod=register&amp;referer=<?php echo urlencode($referer); ?>"
                                   <?php if (!empty($_GET['inajax'])) { ?>onclick="_login.register(<?php echo $referer; ?>);return false;"<?php } ?>
                                   title="<?php echo $_G['setting']['reglinkname']; ?>"><?php echo $_G['setting']['reglinkname']; ?></a>
                                <span class="spacer  pull-right">|</span>
                            <?php } ?>
                            <a class="pull-right"
                               href="user.php?mod=login&amp;op=logging&amp;action=lostpasswd">找回密码</a>
                            <!--                            <img src="avatar.php?uid=1">-->
                        </div>
